...
(async () => {
    ...
    const missing_locale_icon =
        '<i class="fa-light fa-language missing-locale"></i>';
    const default_profile_picture =
        SITE_URL + "assets/images/profile-picture.jpg";
    ...
    function init_Content_Language(language_tabs_container) {
        const language_tab =
            language_tabs_container?.closest('[aria-name="tab"]');
        const language_tabs = language_tab?.querySelectorAll(
            '[aria-name="tabs"][tab-name="language"] li[data-tab]'
        );
    ...
    const language_tabs_containers = document.querySelectorAll(
        '[tab-name="language"][aria-name="tabs-container"]'
    );
    ...
    const initMethod =
        typeof dispatch_keyup_event.initKeyboardEvent !== "undefined"
            ? "initKeyboardEvent"
            : "initKeyEvent";
        ...
        return new Promise((resolve) => {
            ...
            if (action === true) {
                ...
                if (typeof obj.instantRemove !== "undefined") {
                    if (
                        typeof obj.instantRemove.action !== "undefined" &&
                        obj.instantRemove.action === true &&
                        obj.instantRemove.timeout !== "undefined"
                    ) {
                        setTimeout(() => {
                            preLoader.remove();
                ...
                if (typeof selector === "string" && selector.length > 0) {
                    if (selector === "*") {
                        ...
                        preLoader = document.querySelectorAll(
                            `${selector.split("*")[0]} .${css}`
                        );
    ...
    function responsive() {
        ...
        device.isMobile =
            window.screen.width <= device.mobile ||
            window.innerWidth <= device.mobile
                ? true
                : false;
        device.isTab =
            window.screen.width <= device.tab || window.innerWidth <= device.tab
                ? true
                : false;
        device.isDesktop =
            window.screen.width > device.tab || window.innerWidth <= device.tab
    ...
    window.addEventListener("resize", (event) => {
        ...
        const blurScreenBody = document.querySelector(
            `.${tempScreen}.clickable`
        );
        if (device.isTab && expandSidebar.checked === true) {
            Blank_Screen(true, ["clickable"]);
        } else if (
            blurScreenBody &&
            !blurScreenBody.classList.contains("popup")
        ) {
            Blank_Screen(false);
        }
    ...
    function hide_submenu(event) {
        ...
        const expand_fold =
            event.type == "click" &&
            (event.target ==
                document.querySelector('label[for="expand_sidebar"]') ||
                event.target == document.getElementById("expand_sidebar"));
    ...
    document
        .querySelectorAll('.admin:not([data-expand="true"]) li.has-submenu')
        .forEach((submenu) => {
    ...
    function modal(action, obj = {}) {
        if (action === true) {
            const context =
                typeof obj.context !== "undefined" && obj.context.length > 0
                    ? ` data-context="${obj.context}"`
                    : "";
            const css =
                typeof obj.css !== "undefined" && obj.css.length > 0
                    ? ` class="${obj.css}"`
                    : "";
            ...
            if (Modal.querySelector(".modal-content")) {
                if (typeof obj.HTML !== "undefined" && obj.HTML === true) {
                    Modal.querySelector(".modal-content").insertAdjacentHTML(
                        "beforeend",
                        obj.content
                    );
                } else {
            ...
            setTimeout(() => {
                ...
                if (preLoader) {
                    setTimeout(
                        ...
                        typeof obj.preLoaderStartTimeout !== "undefined" &&
                            typeof obj.preLoaderStartTimeout === "number"
                            ? obj.preLoaderStartTimeout
                            : 500
                    );
                }
            }, 50);
    ...
    document.addEventListener("click", (event) => {
        ...
        // Hide/Remove Popups
        if (
            event.target.name == "close-popup" ||
            (event.target.classList.contains("content-blank") &&
                event.target.classList.contains("clickable"))
        ) {
            Blank_Screen(false);
            document
                .querySelectorAll('[aria-name="popup"]')
                .forEach((popup) => {
                    const popup_wrap = popup.querySelector(".admin-popup-wrap");
            ...
            document
                .querySelectorAll(
                    '[aria-name="media-selector-referrence"][media-referrence]'
                )
                .forEach((referrence) => {
    ...
    if (addUser) {
        addUser.addEventListener("click", (event) => {
            ...
            const add_user_form = document.querySelector(
    ...
    if (dataTable) {
        dataTable.addEventListener("click", (event) => {
            ...
            const filled = "filled";
            const SiblingViewStat =
                event.target.name === "delete" &&
                typeof event.target.parentNode.children[0] !== "undefined"
                    ? event.target.parentNode.children[0].classList.contains(
                          filled
                      )
                    : false;
            const SiblingDeleteStat =
                event.target.name === "view" &&
                typeof event.target.parentNode.children[1] !== "undefined"
                    ? event.target.parentNode.children[1].classList.contains(
                          filled
                      )
                    : false;
            ...
            const viewTR =
                dataTR.tagName === "TR" &&
                typeof dataTR.dataset.action !== "undefined" &&
                dataTR.dataset.action === "view";
            ...
            if (
                ...
                (event.target.tagName === "BUTTON" &&
                    (event.target.name === "view" ||
                        event.target.name === "delete" ||
                        event.target.name === "expand"))
            ) {
                const selectedViewButton = document.querySelector(
                    "table.data-table tbody button[name=view]." + filled
                );
                const selectedDeleteButtonSelector = `table.data-table tbody button[name=delete].${filled}`;
                let selectedDeleteButton = document.querySelector(
                    selectedDeleteButtonSelector
                );
                ...
                if (viewTR || event.target.name === "view") {
                    ...
                    if (selectedDeleteButton && SiblingDeleteStat)
                        selectedDeleteButton.classList.remove(filled);

                    if (
                        selectedViewButton === event.target &&
                    ...
                    event.target
                        .closest("table")
                        .classList.contains("responsive-table")
                ) {
                    const expanded = "expanded";
                    if (
                        event.target.closest("tr").classList.contains(expanded)
                    ) {
    ...
    // Close data card
    const dataCardcloseBtn = document.querySelector(
        'button[name="hide-data-card"]'
    );
    if (dataCardcloseBtn) {
        dataCardcloseBtn.addEventListener("click", (event) => {
            document.body.classList.remove("view-data-card");
            const buttons = document.querySelectorAll(
                "table.data-table button[name=view]"
            );
    ...
    function responsiveTable(event = null) {
        setTimeout(() => {
            ...
            if (userList && userList.mobileView === true) {
                ...
                const selectedDeleteButton = document.querySelector(
                    "table.data-table tbody button[name=delete].filled"
                );
    ...
    document.querySelectorAll("table.data-table").forEach((table) => {
        table.querySelectorAll("tbody tr").forEach((tr) => {
            Object.values(tr.children).forEach((td, i) => {
                const title = table.querySelector(
        ...
        if (
            ...
            if (event.target.classList.contains(deleting)) {
                ...
                if (table && tr) {
                    ...
                    let data_basecol_optional = table.getAttribute(
                        "data-basecol-optional"
                    );
                    if (tr && data_single && data_basecol) {
                        let data_name_col = tr.querySelector(
                            "td:nth-child(" + data_basecol + ")"
                        );
                        data_name_col = data_name_col
                            ? data_name_col
                            : tr.querySelector(
                                  "th:nth-child(" + data_basecol + ")"
                              );
                        let item = 0;
                        ...
                        if (data_name_col?.firstChild || item > 0) {
                            if (
                                ...
                                notification.options = {
                                    title: "Sorry!",
                                    description:
                                        "You can't delete yourself account.",
                                };
                            } else {
                                const data_single_title =
                                    data_single.charAt(0).toUpperCase() +
                                    data_single.slice(1);
                                data_basecol_optional = data_basecol_optional
                                    ? tr.querySelector(
                                          "td:nth-child(" +
                                              data_basecol_optional +
                                              ")"
                                      )?.firstChild.data
                                    : null;
                                notification.action = true;
                                notification.options = {
                                    ...
                                    description: `Are you sure to delete the ${data_single}: <strong>${
                                        data_name_col?.firstChild
                                            ? data_name_col.firstChild.data
                                            : item
                                    }</strong> ${
                                        data_basecol_optional
                                            ? "(" + data_basecol_optional + ")"
                ...
                // Notification
                if (
                    typeof notification.action !== "undefined" &&
                    notification.action === true &&
                    typeof notification.stay !== "undefined" &&
                    typeof notification.options !== "undefined"
                ) {
                    ...
                    notification.triggered.notification.nodes[0].addEventListener(
                        "click",
                        (event) => {
                            if (
        ...
        addData.addEventListener("click", function () {
            XHR.onreadystatechange = function () {
                if (this.readyState == 4 && this.status == 200) {
                    ...
                    if (form) {
                        const signature =
                            form.querySelector(".field.signature");
        ...
        if (typeof items === "object" && items?.length > 0) {
            items?.forEach((item) => {
                if (
                    ...
                    obj?.placeholder?.trim()?.length > 0
                ) {
                    if (item?.value?.trim().length > 0)
                        item.value = obj.placeholder;
                    ...
                    if (value?.length > 0) {
                        ...
                        item.label =
                            t_label === value && item?.value?.length > 0
                                ? item?.value + " " + missing_locale_icon
                                : t_label;
    ...
    function updateChoice(choice, obj = {}) {
        if (Object.keys(obj).length > 0) {
            ...
            if (choice?._placeholderValue && obj?.placeholder)
                choice._placeholderValue = obj?.placeholder;

            // Update Choice Config Text
            const { i18next } = obj;
            if (i18next && choice?.config) {
                const obj_i18n = choice?.config?.classNames?.i18next;
                Object.keys(obj_i18n).forEach(
                    (t) => (choice.config[t] = i18next.t(obj_i18n[t]))
                );
            }
            ...
            // Update Choice Placeholder
            updateChoiceSearchPlaceholder(
                choice,
                choice?.config?.searchPlaceholderValue
            );
        }
    }
    ...
    if (user_access_roles)
        choices.push(
            Choice(user_access_roles, user_access_roles_list, {
                i18next: i18next,
            })
        );
    const filter_user_access_roles = document.getElementById("filter-role");
    if (filter_user_access_roles)
        choices.push(
            Choice(filter_user_access_roles, user_access_roles_list, {
                i18next: i18next,
            })
        );

    // Address Dynamic Selection
    function addressDynamicSelection(
        division = null,
        district = null,
        upzilla = null,
        union = null,
        ...
        division?.passedElement?.element?.addEventListener(
            "change",
            (event) => {
                if (event?.target?.value?.trim()?.length > 0) {
                    ...
                    district?.disable();
                    upzilla?.setChoiceByValue("");
                    upzilla?.disable();
                    union?.setChoiceByValue("");
                    union?.disable();
        ...
        district?.passedElement?.element?.addEventListener(
            "change",
            (event) => {
                if (event?.target?.value?.trim()?.length > 0) {
                    upzilla?.enable();
    ...
    const divisions = document.querySelectorAll(
        '#division:not([data-select="import-column"])'
    );
                    ...
                    [
                        {
                            value: "dhaka",
                            customProperties: {
                                locale: { en: "Dhaka", bn: "ঢাকা" },
                            },
                        },
                        {
                            value: "chittagong",
                            customProperties: {
                                locale: { en: "Chittagong", bn: "চট্টগ্রাম" },
                            },
                        },
                        {
                            value: "rajshahi",
                            customProperties: {
                                locale: { en: "Rajshahi", bn: "রাজশাহী" },
                            },
                        },
                        {
                            value: "khulna",
                            customProperties: {
                                locale: { en: "Khulna", bn: "খুলনা" },
                            },
                        },
                        {
                            value: "jessore",
                            customProperties: {
                                locale: { en: "Jessore", bn: "যশোর" },
                            },
                        },
                        {
                            value: "barishal",
                            customProperties: {
                                locale: { en: "Barishal", bn: "বরিশাল" },
                            },
                        },
                        {
                            value: "sylhet",
                            customProperties: {
                                locale: { en: "Sylhet", bn: "সিলেট" },
                            },
                        },
                        {
                            value: "rangpur",
                            customProperties: {
                                locale: { en: "Rangpur", bn: "রংপুর" },
                            },
                        },
                    ],
                    locale?.lang,
    ...
    const committee_types = [
        {
            value: "union-ward-executive",
            label: "Union Ward Executive",
            customProperties: {
                locale: { bn: "ইউনিয়ন ওয়ার্ড নির্বাহী কমিটি" },
            },
        },
        {
            value: "union-municipal-executive",
            label: "Union Municipal Executive",
            customProperties: {
                locale: {
                    bn: "ইউনিয়ন এবং মহানগর/পৌরসভা ওয়ার্ড নির্বাহী কমিটি",
                },
            },
        },
        {
            value: "union-council-executive",
            label: "Union Council Executive",
            customProperties: {
                locale: { bn: "ইউনিয়ন কাউন্সিল ও ইউনিয়ন নির্বাহী কমিটি" },
            },
        },
        {
            value: "upzilla-executive",
            label: "Upzilla Executive",
            customProperties: {
                locale: { bn: "উপজেলা/মহানগর থানা/পৌর নির্বাহী কমিটি" },
            },
        },
        {
            value: "municipal-council-executive",
            label: "Municipal Council Executive",
            customProperties: {
                locale: { bn: "পৌরসভা কাউন্সিল ও পৌরসভা নির্বাহী কমিটি" },
            },
        },
        {
            value: "metropolitan-executive",
            label: "Metropolitan Executive",
            customProperties: { locale: { bn: "মহানগর নির্বাহী কমিটি" } },
        },
        {
            value: "distrcit-executive",
            label: "Distrcit Executive",
            customProperties: { locale: { bn: "জেলা নির্বাহী কমিটি" } },
        },
        {
            value: "national-council",
            label: "National Council",
            customProperties: { locale: { bn: "জাতীয় কাউন্সিল" } },
        },
        {
            value: "national-standing",
            label: "National Standing",
            customProperties: { locale: { bn: "জাতীয় স্থায়ী কমিটি" } },
        },
        {
            value: "national-exectuive",
            label: "National Exectuive",
            customProperties: { locale: { bn: "জাতীয় নির্বাহী কমিটি" } },
        },
        {
            value: "chairman-advisor-council",
            label: "Chairman Advisor Council",
            customProperties: {
                locale: { bn: "চেয়ারম্যানের উপদেষ্টা কাউন্সিল" },
            },
        },
        {
            value: "thematic-sub-committee",
            label: "Thematic Sub Committee",
            customProperties: { locale: { bn: "বিষয়ভিত্তিক উপ-কমিটি" } },
        },
        {
            value: "parliamentary-board",
            label: "arliamentary Board",
            customProperties: { locale: { bn: "পার্লামেন্টারি বোর্ড" } },
        },
        {
            value: "parliamentary-party",
            label: "Parliamentary Party",
            customProperties: { locale: { bn: "পার্লামেন্টারি পার্টি" } },
        },
        {
            value: "abroad",
            label: "Abroad Organization",
            customProperties: { locale: { bn: "প্রবাসে সংগঠন" } },
        },
        {
            value: "special-assistant",
            label: "",
            customProperties: { locale: { bn: "চেয়ারম্যানের বিশেষ সহকারী" } },
        },
    ];
    // Wing Types
    const wing_types = [
    ...
    if (committee_type) {
        committee_type = Choice(committee_type, committee_types, {
            search: true,
            i18next: i18next,
        });
    ...
    if (committee_category) {
        ...
        select.addEventListener("change", (event) => {
            ...
            committee_type.setChoices(
                val === "general" ? committee_types : wing_types,
                "value",
                "label",
                val === "general" ? wing_types : committee_types
            );
        });
    }
    ...
    const member_roles = [
        {
            value: "president",
            label: "President",
            customProperties: { locale: { bn: "প্রেসিডেন্ট" } },
        },
        {
            value: "vice-president",
            label: "Vice President",
            customProperties: { locale: { bn: "ভাইস প্রেসিডেন্ট" } },
        },
        {
            value: "chairperson",
            label: "Chairperson",
            customProperties: { locale: { bn: "চেয়ারপার্সন" } },
        },
        {
            value: "secretary-general",
            label: "Secretary General",
            customProperties: { locale: { bn: "মহাসচিব" } },
        },
        {
            value: "chairman",
            label: "Chairman",
            customProperties: { locale: { bn: "সভাপতি" } },
        },
        {
            value: "vice-chairman",
            label: "Vice Chairman",
            customProperties: { locale: { bn: "সহ-সভাপতি" } },
        },
        {
            value: "general-secretary",
            label: "General Secretary",
            customProperties: { locale: { bn: "সাধারণ সম্পাদক" } },
        },
        {
            value: "joint-editor",
            label: "Joint Editor",
            customProperties: { locale: { bn: "যুগ্ম সম্পাদক" } },
        },
    ];
    const roles = document.querySelectorAll("#role");
    roles.forEach((role) => {
        choices.push(
            Choice(role, member_roles, { search: true, i18next: i18next })
        );
    });
    ...
    const select_date_formats = document.querySelectorAll(
        'select[name="select-date-format"]'
    );
    ...
    const select_time_formats = document.querySelectorAll(
        'select[name="select-time-format"]'
    );
    ...
    if (content_bn) {
        ...
        tinymce.init({
            ...
            toolbar:
                "undo redo | formatselect | bold italic | alignleft aligncentre alignright alignjustify | indent outdent | bullist numlist",
            content_style:
    ...
    document.addEventListener("addItem", (event) => {
        const committee_member = event.target.closest(
            '[aria-name="committee-member"]'
        );
        const member_name = committee_member?.querySelector(
            'select[one][name="committee-member-name"]'
        );
        const this_role = committee_member?.querySelector(
            'select[one][name="committee-member-role"]'
        );
        if (
            committee_member &&
            (event.target === member_name || event.target === this_role)
        ) {
            ...
            const this_choice_role = findPairOfMember(
                members_data,
                "role",
                this_role
            );
            const this_mobile = committee_member.querySelector(
                'input[type=number][name="committee-member-mobile"]'
            );
            const this_email = committee_member.querySelector(
                'input[type=email][name="committee-member-email"]'
            );

            if (event.target === member_name) {
                if (this_img) {
                    if (member?.img?.trim()?.length > 0) {
                        this_img.style.backgroundImage =
                            "url(" + member.img + ")";
                        target_img?.classList?.add("visible");
                    } else {
                        target_img?.classList?.remove("visible");
                        setTimeout(() => {
                            this_img?.style?.removeProperty("background-image");
                        }, 350);
                    }

                    // Unlisted member
                    const unlisted_member_name_en =
                        committee_member.querySelector(
                            'input[name="unlisted-member-name-en"]'
                        );
                ...
                if (this_choice_role) {
                    if (member_name?.value.trim()?.length === 0)
                        this_choice_role.disable();
                    else this_choice_role.enable();
                    if (typeof member?.role !== "undefined")
                        this_choice_role.setChoiceByValue(member.role);
                    else this_choice_role?.setChoiceByValue("");
                }
            }
            if (this_mobile) {
                this_mobile.disabled =
                    typeof member?.mobile !== "undefined" ||
                    this_role?.value?.length > 0
                        ? false
                        : true;
                this_mobile.value =
                    typeof member?.mobile !== "undefined"
            ...
            if (this_email) {
                this_email.disabled =
                    ...
                    this_role?.value?.length > 0
                        ...
                        : true;
                this_email.value =
                    typeof member?.email !== "undefined" ? member.email : null;
            }
        }
    });
    ...
    function countCommitteeMemberSL() {
        const committee_members = document.querySelectorAll(
            ".committee-members > .committee-member:not(#add-member)"
        );
    ...
    const member_list = [
        {
            value: "member-id",
            label: '<img src="../assets/images/users/fakhrul.jpg" alt="Mirza Fakhrul Islam Alamgir"> Mirza Fakhrul Islam Alamgir',
            customProperties: {
                img: "../assets/images/users/fakhrul.jpg",
                role: "secretary-general",
                mobile: "01780000009",
                email: "mail@bnpbd.org",
            },
        },
        {
            value: "member-id",
            label:
                ...
                '" alt="Mirza Fakhrul Islam Alamgir"> Mirza Fakhrul Islam Alamgir',
        },
    ];
    if (add_member) {
        add_member.addEventListener("click", async (event) => {
            const committee_members =
                document.querySelector(".committee-members");
            const members = committee_members.querySelectorAll(
                ".committee-member:not(#add-member)"
            );
            const html = await LoadHTML(
                "ul.committee-members > .committee-member"
            );
            if (html) {
                const count = html.querySelector(
                    ".committee-member > .count-sl"
                );
                if (count) count.innerText = members.length + 1;
                committee_members.insertBefore(html, add_member);
                const member = html.querySelector(
                    'select[one][name="committee-member-name"]'
                );
                const member_role = html.querySelector(
                    'select[one][name="committee-member-role"]'
                );
                const member_mobile = html.querySelector(
                    'input[name="committee-member-mobile"]'
                );
                const member_email = html.querySelector(
                    'input[name="committee-member-email"]'
                );

                if (LANG === "bn") {
                    if (member?.querySelector('option[value=""]'))
                        member.querySelector('option[value=""]').innerHTML =
                            "সদস্যের নাম";
                    if (member_role?.querySelector('option[value=""]'))
                        member_role.querySelector(
                            'option[value=""]'
                        ).innerHTML = "পদবী";
                ...
                if (member && member_role && member_list && member_roles) {
                    const name = Choice(member, member_list, {
                        disabled: false,
                        add_data: true,
                        search: true,
                        reset: true,
                    ...
                    const role = Choice(member_role, member_roles, {
                        add_data: true,
                        search: true,
                        reset: true,
                        i18next: i18next,
                ...
                if (typeof DragNDrop === "function")
                    DragNDrop(countCommitteeMemberSL);
            }
        });
    }
    ...
    if (event_leader)
        choices.push(
            Choice(event_leader, member_list, {
                search: true,
                reset: true,
                i18next: i18next,
            })
    ...
    document.addEventListener("click", (event) => {
        const add_choice_manually = document.getElementById(
            "add-choice-manually"
        );
        if (event.target == add_choice_manually) {
            ...
            if (choice_list) {
                ...
                const choices_item =
                    choice_list.querySelector(".choices__item");
                if (select && choices_item) {
                    const data = event.target.getAttribute("data-value");
                    if (data.length > 0) {
                        // Reset search
                        const search = choice_list.querySelector(
                        ...
                        let member_name =
                            choice_list.querySelector("select[one]");
                        member_name = findPairOfMember(
                            members_data,
                            "name",
                            member_name
                        );
                        member_name.setChoices([
                            {
                                value: data,
                                label:
                                    '<img src="' +
                                    ...
                                    '" class="unlisted"> ' +
                                    data,
                                customProperties: { unlisted: true },
                            },
                        ]);
    ...
    document.addEventListener("keyup", (event) => {
        if (event.key === "Enter") {
            const choices_list_dropdown = event.target.closest(
                ".choices__list--dropdown"
            );
            if (choices_list_dropdown) {
                const search = choices_list_dropdown.querySelector(
                    'input[type="search"]'
                );
                const add_choice_manually = choices_list_dropdown.querySelector(
                    "#add-choice-manually"
                );
    ...
    const filter_otpions = document.querySelector(
        ".data-filter.buttoned-filter"
    );
    ...
    document.addEventListener("click", async (event) => {
        if (
            event.target.tagName === "BUTTON" &&
            event.target.name === "media-selector"
        ) {
            const media_selector_referrence = event.target.closest(
                '[aria-name="media-selector-referrence"]'
            );
            if (media_selector_referrence) {
                media_selector_referrence.setAttribute(
                    "media-referrence",
                    "active"
                );

                const media_selector = await LoadHTML(
                    '[aria-name="media-selector"]'
                );
                const allowedFiles = event.target.getAttribute("allowed-files");
                const url = event.target.getAttribute("url");
                const unknownSource =
                    event.target.getAttribute("unknown-source");

                if (allowedFiles?.length > 1)
                    media_selector.setAttribute("allowed-files", allowedFiles);
                if (JSON.parse(unknownSource) === true)
                    media_selector
                        ?.querySelector('[aria-name="API"]')
                        ?.setAttribute("unknown-source", unknownSource);
                if (JSON.parse(url) !== true)
                    media_selector
                        ?.querySelector('[aria-name="API"]')
                        ?.remove();

                const add_media = media_selector.querySelector(
                    ".col-right.add-media"
                );
                if (add_media) {
    ...
    if (select_all_media) {
        select_all_media.addEventListener("click", (event) => {
            if (
                select_all_media == event.target &&
                select_media?.getAttribute("data-select-media") === "true" &&
                select_media?.classList.contains("active")
            ) {
                const medias = document.querySelectorAll(
                    "ul.medias > li[data-id]"
                );
    ...
    document.addEventListener("click", (event) => {
        ...
        if (
            ...
            const media_selector = event.target.closest(
                'div[aria-name="media-selector"]'
            );
            const media_selector_url = document?.querySelector(
                '[aria-name="media-selector"] #media-selector-url'
            );
            ...
            if (
                ...
                const selected_medias = document.querySelectorAll(
                    'ul.medias > li[data-id][aria-checked="true"]'
                );
                if (select_all_media) {
            ...
            // Media Selector Page
                ...
                const insert_btn = media_selector.querySelector(
                    "#insert-selected-media"
                );
                if (insert_btn) {
                    const selected_medias = media_selector.querySelectorAll(
                        'ul.medias > li[data-id][aria-checked="true"]'
                    );
        ...
        // Insert media
            ...
            event.target.tagName == "BUTTON" &&
            event.target.id == "insert-selected-media" &&
            !event.target.disabled
        ) {
            const refference_active_aria = document.querySelector(
                '[aria-name="media-selector-referrence"][media-referrence="active"]'
            );
            if (refference_active_aria) {
                ...
                const selected_items = medias.querySelectorAll(
                    'li[aria-checked="true"]'
                );
                const selected_item = medias.querySelector(
                    'li[aria-checked="true"]'
                );
                const source_file =
                    selected_item?.querySelector(".source-file");

                const url_source_file = media_selector?.querySelector(
                    '[aria-name="media-selector-footer"] input[type="url"]#media-selector-url'
                );
                const url_source_file_value = url_source_file?.value?.trim();

                const isURLSelected =
                    selected_items.length === 0 &&
                    url_source_file_value?.length > 0
                        ? true
                        : false;
                ...
                let selected_media = {
                    id: isMediaSelected
                        ? selected_item?.getAttribute("data-id")
                        : url_source_file_value,
                    source: isMediaSelected
                        ? source_file?.getAttribute("data-source")
                        : url_source_file_value,
                    "file-ext": isMediaSelected
                        ? source_file?.getAttribute("file-ext")
                        : media_selector
                              ?.querySelector(
                                  '[aria-name="media-selector-footer"] div[file-ext]'
                              )
                              ?.getAttribute("file-ext"),
                };
                if (isMediaSelected) {
                    if (!selected_media?.source)
                        selected_media.source =
                            source_file?.getAttribute("src");
                    if (!selected_media?.["file-ext"])
                        selected_media["file-ext"] = urlFileExtension(
                            selected_media?.source
                        );
                }

                const input_media_id = refference_active_aria.querySelector(
                    'input[name="media-id"]'
                );
                const to_source_file =
                    refference_active_aria.querySelector(".source-file");
                const placeholder = to_source_file?.querySelector(
                    ".content-placeholder"
                );
                placeholder?.style?.removeProperty("background-image");

                if (placeholder) {
                    const allowedFiles = media_selector
                        ?.getAttribute("allowed-files")
                        ?.split(" ");
                    if (
                        isAllowedFile(
                            selected_media?.["file-ext"],
                            allowedFiles
                        )
                    ) {
                        if (
                            ((isMediaSelected &&
                                isInteger(selected_media?.id)) ||
                                isURLSelected) &&
                            selected_media?.id &&
                            selected_media?.source &&
                            selected_media?.["file-ext"] &&
                            ...
                            if (isImage(selected_media?.["file-ext"])) {
                                if (placeholder?.tagName === "INPUT") {
                                    ...
                                    placeholder.style.backgroundImage =
                                        "url(" + selected_media.source + ")";
                                }
                            }
                            to_source_file.classList.add("visible");

                            // Close popup
                            const cancel_btn = event.target
                                .closest("div")
                                ?.querySelector('button[name="close-popup"]');
                            cancel_btn?.click();
                        }
                    } else {
        ...
        // Reset media
            ...
            event.target.tagName === "BUTTON" &&
            (event.target.name === "reset-media-selector" ||
                event.target.name === "reset-media-selector-default")
        ) {
            const media_selector_refference = event.target.closest(
                '[aria-name="media-selector-referrence"]'
            );
            const input_media_id = media_selector_refference?.querySelector(
                'input[name="media-id"]'
            );
            const to_source_file =
                media_selector_refference?.querySelector(".source-file");
            const placeholder = to_source_file?.querySelector(
                ".content-placeholder"
            );
            ...
            if (input_media_id && placeholder) {
                ...
                if (event.target.name === "reset-media-selector") {
                    ...
                    const source_value =
                        event.target?.getAttribute("source-value");
        ...
        // Clear Media Selector URL
            ...
            event.target.tagName === "BUTTON" &&
            event.target.name === "clear-media-selector-url"
        ) {
            const media_selector_footer = event.target.closest(
                'div[aria-name="media-selector-footer"]'
            );
            const media_selector_url = media_selector_footer?.querySelector(
                "#media-selector-url"
            );
            const external_source_view = media_selector_footer?.querySelector(
                ".external-source-view"
            );
            if (media_selector_url) {
                media_selector_url.value = null;
                media_selector_url?.dispatchEvent(
                    new Event("change", { bubbles: true })
                );
        ...
        // Media Grid view / List View
            ...
            event.target.tagName === "BUTTON" &&
            event.target.closest(".grid-list-view") &&
            (event.target.id === "media-grid" ||
                event.target.id === "media-list")
        ) {
            if (!event.target.classList.contains("active")) {
                ...
                if (
                    medias?.getAttribute("data-view") !== "list" &&
                    event.target.id === "media-list" &&
                    event.target == list_btn
                ) {
    ...
    function APIRequest(event, req = {}) {
        return new Promise((resolve, reject) => {
            ...
            const requst_status = API.querySelector(
                '[aria-name="api-request-status"]'
            );
            const XHR = new XMLHttpRequest();
            ...
            if (requst_status && req?.URL?.length > 0) {
                ...
                XHR.onload = () => {
                    ...
                    if (file_extension.includes("drive"))
                        file_extension = content_type?.split("/")?.at(-1);
                    ...
                    if (XHR.status < 400) {
                        if (isAllowedFile(file_extension, req?.files)) {
                            ...
                            API_Error(
                                requst_status,
                                1000,
                                API_Error_Content + " File-type not allowed"
                            );
                            reject({
                                success: false,
                                XHR: XHR,
                ...
                XHR.onerror = () => {
                    ...
                    const isAllowed = Object.keys(allowedURLS).filter((key) =>
                        req?.URL?.includes(allowedURLS[key])
                    );
                    if (
                        req?.unknsource === true &&
                        XHR.readyState === 4 &&
                        ...
                        const notification = GrowlNotification.notify({
                            type: "default",
                            showProgress: true,
                            closeTimeout: 10000,
                            title: "External link",
                            description:
                                "You're trying to add unverified source, would you agree with this?",
                            position: "top-right",
                            showButtons: true,
                            buttons: {
                                action: {
                                    ...
                                    callback: () => {
                                        requst_status.innerHTML = null;
                                        requst_status?.classList?.remove(
                                            "pending"
                                        );
                                        resolve({
                                            success: true,
    ...
    document.addEventListener("keyup", async (event) => {
        if (event.target.id === "media-selector-url") {
            ...
            const external_source_view = API?.querySelector(
                ".external-source-view"
            );
            const placeholder = external_source_view?.querySelector(
                ".content-placeholder"
            );
            ...
            APIRequest(event, req)
                .then((response) => {
                    if (response?.success === true) {
                        if (external_source_view && placeholder) {
                            ...
                            external_source_view?.setAttribute(
                                "file-ext",
                                file_extension
                            );
                            insert_btn?.removeAttribute("disabled");
                ...
                .catch((response) => {
                    if (response?.success === false) {
                        external_source_view?.removeAttribute(
                            "file-ext",
                            "image"
                        );
                        placeholder.style.removeProperty("background-image");
    ...
    function multipleTableBulkSelection(tables, table = null) {
        tables.forEach((this_table) => {
            if (this_table !== table || table === null) {
                const selected_checkbox = this_table.querySelectorAll(
                    "thead tr th:nth-child(1) input[type=checkbox], tbody tr td:nth-child(1) input[type=checkbox]"
                );
    ...
    document.addEventListener("click", (event) => {
        ...
        if (table) {
            ...
            if (
                event.target === select_all_data ||
                event.target.name === "select-individual-data"
            ) {
                // Check if the page contains multiple data tables
                const tables = document.querySelectorAll("table.data-table");
                multipleTableBulkSelection(tables, table);

                let select_individuals = table?.querySelectorAll(
                ...
                if (event.target == select_all_data) {
                    rows.forEach((row) => {
                        const checkbox = row.querySelector(
                            "td:nth-child(" + column + ") input[type=checkbox]"
                        );
                    ...
                    select_individuals = table?.querySelectorAll(
                        "tbody tr td:nth-child(" +
                            column +
                            ") input[type=checkbox]:checked"
                    );
                ...
                // Select individual
                    ...
                    if (select_all_data) {
                        select_all_data.checked =
                            rows.length === select_individuals.length
                                ? true
                                : false;
                    }
                }

                const bulk_action_btn = document.querySelector(
        ...
        if (
            event.target.tagName === "BUTTON" &&
            event.target.id === "data-bulk-action"
        ) {
            let aria_bulk_action = event.target.closest(
                '[aria-name="bulk-action"]'
            );
            aria_bulk_action = event.target.closest(".data-bulk-action");
            const action_type = aria_bulk_action?.getAttribute("action-type");
            const action =
                aria_bulk_action?.querySelector("#data-bulk-options")?.value;
            if (action?.trim()?.length === 0) {
                ...
                if (action === "delete") {
                    const deleting = "deleting";
                    const tables =
                        document.querySelectorAll("table.data-table");
                    if (tables.length > 0) {
                        tables.forEach((table) => {
                            const checkboxes = table.querySelectorAll(
                                'tbody tr td:first-child input[type="checkbox"]:checked'
                            );
                            if (checkboxes.length > 0) {
                                checkboxes.forEach((checkbox, i) => {
                                    if (checkbox?.checked === true) {
                                        if (action_type === "button-updater") {
                        ...
                        document
                            .querySelectorAll("#select-all-data")
                            .forEach(
                                (select_all) => (select_all.checked = false)
                            );
                    }
                }
            }
    ...
    if (data_bulk_options) {
        ...
        if (page.classList.contains("posts")) {
            options.push({
                value: "category",
                customProperties: { i18next: "action.category" },
            });
        }
        options.push({
            value: "delete",
            customProperties: { i18next: "action.delete" },
        });
    ...
    // START - Choices
    const filter_post_page_status = document.getElementById(
        "filter-post-page-status"
    );
    if (filter_post_page_status) {
        choices.push(
            Choice(
                filter_post_page_status,
                [
                    {
                        label: "Status",
                        id: 1,
                        ...
                        choices: [
                            {
                                value: "published",
                                customProperties: {
                                    i18next: "status.published",
                                },
                            },
                            {
                                value: "draft",
                                customProperties: { i18next: "status.draft" },
                            },
                            {
                                value: "scheduled",
                                customProperties: {
                                    i18next: "status.scheduled",
                                },
                            },
                        ],
                    },
                        ...
                        choices: [
                            {
                                value: "public",
                                customProperties: {
                                    i18next: "visibility.public",
                                },
                            },
                            {
                                value: "private",
                                customProperties: {
                                    i18next: "visibility.private",
                                },
                            },
                            {
                                value: "protected",
                                customProperties: {
                                    i18next: "visibility.protected",
                                },
                            },
                        ],
                    },
        ...
        choices.push(
            Choice(
                ...
                [
                    {
                        value: "published",
                        customProperties: { i18next: "status.published" },
                    },
                    {
                        value: "draft",
                        customProperties: { i18next: "status.draft" },
                    ...
                    {
                        value: "scheduled",
                        customProperties: { i18next: "status.scheduled" },
                    },
                ],
    ...
    if (visibiltiy) {
        choices.push(
            Choice(
                ...
                [
                    {
                        value: "public",
                        customProperties: { i18next: "visibility.public" },
                    },
                    {
                        value: "private",
                        customProperties: { i18next: "visibility.private" },
                    },
                    {
                        value: "password-protected",
                        customProperties: {
                            i18next: "visibility.password-protected",
                        },
                    },
                ],
                { reset: false, i18next: i18next }
        ...
        visibiltiy.addEventListener("change", (event) => {
            const protected_aria = event.target
                .closest('[aria-name="status-visibility"]')
                ?.querySelector('[aria-name="password-protected"]');
    ...
    const category_list = [
        {
            value: "default",
            customProperties: { locale: { en: "Default", bn: "ডিফল্ট" } },
        },
    ];
    let parent_category = document.getElementById("parent-category");
    if (parent_category) {
        parent_category = Choice(parent_category, category_list, {
            search: true,
            i18next: i18next,
        });
    ...
    if (post_categories) {
        post_categories = Choice(post_categories, category_list, {
            search: true,
            i18next: i18next,
        });
    ...
    document.addEventListener("click", (event) => {
        if (event.target.tagName === "BUTTON") {
            ...
            tr = tr
                ? tr
                : document.querySelectorAll(".data-content table tbody tr");
            ...
            let category_aria = event.target?.closest(
                '[aria-name="add-category"]'
            );
            category_aria = category_aria
                ? category_aria
                : event.target?.closest('[aria-name="edit-category"]');
            const category_form = category_aria?.querySelector(
                '[aria-name="category-form"]'
            );

            const input_category_name_bn =
                category_form?.querySelector("#category-name-bn");
            const input_category_name_en =
                category_form?.querySelector("#category-name-en");
            const input_category_slug =
                category_form?.querySelector("#category-slug");
            const input_category_parent = parent_category;
            const input_edit_category_id = category_form?.querySelector(
                "#input-edit-category"
            );
            const btn_main = category_form?.querySelector("button.main");
            const btn_field = category_form?.querySelector(
                ".cancel-onpage-action"
            );

            const action_edit_verification =
                tr &&
                target_data_id &&
                (category_name_bn || category_name_en) &&
                category_slug &&
                category_parent &&
                category_aria &&
                (input_category_name_bn || input_category_name_en) &&
                input_category_slug &&
                input_category_parent &&
                input_edit_category_id &&
                btn_main &&
                btn_field;
            const action_cancel_verification =
                tr?.length > 0 &&
                input_category_slug &&
                input_category_parent &&
                input_edit_category_id &&
                btn_main;
            if (
                event.target.name === "edit-category" &&
            ...
            // Cancel edit category action
                ...
                event.target.name === "cancel-onpage-action" &&
                action_cancel_verification
            ) {
                tr.forEach((row) => {
    ...
    document.addEventListener("change", (event) => {
        if (event.target.closest(".slides")) {
            if (
                ...
                event.target.name === "content-placeholder"
            ) {
                const tr = event.target.closest("tr");
                if (event.target?.classList?.contains("background")) {
                    const slide_preview_wrap = tr?.querySelector(
                        "td .slide-preview-wrap"
                    );
                    if (slide_preview_wrap) {
                        event.target.value = event.target.value?.trim();
                        if (event.target?.value?.length > 0) {
                            slide_preview_wrap.style.backgroundImage =
                                "url(" + event.target.value + ")";
                        } else {
                            slide_preview_wrap?.style?.removeProperty(
                                "background-image"
                            );
                        }
                    }
                } else if (event.target?.classList?.contains("logo")) {
                    const slide_preview_logo = tr?.querySelector(
                        "td .slide-preview-logo"
                    );
                    if (slide_preview_logo) {
                        event.target.value = event.target.value?.trim();
                        if (event.target?.value?.length > 0) {
                            slide_preview_logo.style.backgroundImage =
                                "url(" + event.target.value + ")";
                            slide_preview_logo?.classList?.add("visible");
                        } else {
                            slide_preview_logo?.classList?.remove("visible");
                            slide_preview_logo?.style?.removeProperty(
                                "background-image"
                            );
                        }
                    }
    ...
    // Slide Content
    document.addEventListener("keyup", (event) => {
        const slide_content_input = event.target.closest(
            ".slide-content-input"
        );
        if (
            event.target.tagName === "INPUT" &&
            ...
            slide_content_input &&
            (event.target.name === "slide-title" ||
                event.target.name === "slide-description" ||
                event.target.name === "slide-url" ||
                event.target.name === "slide-overlay-opacity")
        ) {
            const tr = event.target.closest("tr");
            const slide_preview_content = tr.querySelector(
                ".slide-preview-content"
            );
            if (slide_preview_content) slide_preview_content.innerHTML = null;

            const slide_title = slide_content_input
                ?.querySelector('input[name="slide-title"]')
                ?.value?.trim();
            const slide_url = slide_content_input
                ?.querySelector('input[name="slide-url"]')
                ?.value?.trim();
            const slide_description = slide_content_input
                ?.querySelector('input[name="slide-description"]')
                ?.value?.trim();

            const a = document.createElement("a");
            const h2 = document.createElement("h2");
            const title_color = slide_content_input.querySelector(
                'input[name="title-color"]'
            );
            const p = document.createElement("p");
            const description_color = slide_content_input.querySelector(
            ...
            if (slide_title.length > 0) {
                ...
                if (title_color?.dataset?.currentColor)
                    h2.style.color = title_color?.dataset?.currentColor;
            ...
            if (slide_title.length > 0 && slide_url.length > 0) {
                ...
                if (title_color?.dataset?.currentColor)
                    a.style.color = title_color?.dataset?.currentColor;
            }
            if (slide_description.length > 0) {
                p.innerText = slide_description;
                slide_preview_content.appendChild(p);
                if (description_color?.dataset?.currentColor)
                    p.style.color = description_color?.dataset?.currentColor;
            }
        }
    });
    ...
    function updateSlideColor(event) {
        ...
        const preview_element = el
            ?.closest("tr")
            ?.querySelector(".slide-preview-wrap");
        if (
            el?.name == "title-color" ||
            el?.name == "description-color" ||
            el?.name == "overlay-bg-left" ||
            el?.name == "overlay-bg-right"
        ) {
            const a = preview_element.querySelector(".slide-preview-content a");
            const h2 = preview_element.querySelector(
            ...
            if (el?.name == "description-color") {
                ...
                const overlay_bg_left = slide_content_input.querySelector(
                    'input[name="overlay-bg-left"]'
                );
                const overlay_bg_right = slide_content_input.querySelector(
                    'input[name="overlay-bg-right"]'
                );
                preview_element.style.backgroundImage =
                    "linear-gradient(to right, " +
                    overlay_bg_left?.value +
                    ", " +
                    overlay_bg_right?.value +
                    ")";
            }
        }
    }
    ...
    if (add_new_slide) {
        add_new_slide.addEventListener("click", async (event) => {
            ...
            init_Content_Language(
                html?.querySelector(
                    '[aria-name="tabs-container"][tab-name="language"]'
                )
            );
            if (typeof i18nTranslate === "function") i18nTranslate(html);

            const slides_table = document.querySelector(
                ".slides table.data-table tbody"
            );
            ...
            const title_color = html.querySelector(
                '.slide-content-input input[name="title-color"]'
            );
            const description_color = html.querySelector(
                '.slide-content-input input[name="description-color"]'
            );
            const overlay_bg_left = html.querySelector(
                '.slide-content-input input[name="overlay-bg-left"]'
            );
            const overlay_bg_right = html.querySelector(
                '.slide-content-input input[name="overlay-bg-right"]'
            );
    ...
    document.addEventListener("click", async (event) => {
        if (event.target.tagName === "LI" || event.target.tagName === "DIV") {
            ...
            const tab_name = event.target
                ?.closest("[tab-name]")
                ?.getAttribute("tab-name");
            ...
            if (tab_aria && tab_name && tabs) {
                ...
                const data_tabs = document.querySelectorAll(
                    '[aria-name="tabs-container"][tab-name="' +
                        tab_name +
                        '"] > [data-tab]'
                );
                data_tabs.forEach((data_tab) => {
                    if (data_tab?.getAttribute("data-tab") === selected_tab) {
                        const tab_heading = data_tab.querySelector(
                            ".tab-container-heading h2"
                        );
                ...
                if (tab_aria?.classList.contains("language-tabs")) {
                    const language_tabs =
                        document.querySelectorAll(".language-tabs");
                    language_tabs.forEach((language_tab) => {
                        if (language_tab !== tab_aria) {
                            const target_language_tab_cotainer =
                                language_tab.querySelector(
                                    '[aria-name="tabs-container"][aria-name="' +
                                        tab_name +
                                        '"] > [data-tab="' +
                                        selected_tab +
                                        '"]'
                                );
                            const language_tab_containers =
                                language_tab.querySelectorAll(
                                    '[aria-name="tabs-container"][aria-name="' +
                                        tab_name +
                                        '"] > [data-tab]'
                                );
                            language_tab_containers.forEach((container) => {
                                if (
                                    container === target_language_tab_cotainer
                                ) {
            ...
            event.target.tagName === "BUTTON" &&
            event.target.name === "add-dbdata"
        ) {
            const data_wrapper = event.target.closest(
                '[aria-name="data-wrapper"]'
            );
            const data_content = data_wrapper?.querySelector(".data-content");
            const table = data_wrapper?.querySelector("table.data-table");
            const tbody = table?.querySelector("tbody");
            const html = await LoadHTML();
            const tr_default = html?.querySelector(
                '[aria-name="dbdata-row"] tbody tr'
            );

            if (data_content && tbody && tr_default) {
                tr_default?.classList.add("new");
                if (event.target?.classList.contains("user-access")) {
                    const user_access_td = html?.querySelector(
                        '[aria-name="dbdata-user-access"] tbody tr td'
                    );
                ...
                data_content.scrollTo({
                    top: tr_default.offsetTop,
                    behavior: "smooth",
                });
    ...
    document.addEventListener("click", async (event) => {
        if (
            event.target?.tagName === "BUTTON" &&
            event.target?.name === "notification"
        ) {
            const send_notification = await LoadHTML(
                '[aria-name="send-notification"]'
            );
            insertPopup(send_notification, ["popup", "white", "clickable"]);
        }
    });
    ...
    document.addEventListener("keyup", (event) => {
        if (
            event.target?.tagName === "INPUT" &&
            event.target.name === "site-title"
        ) {
            const settings = event.target?.closest(".settings");
            const favicon_tab_title = settings.querySelector(
                ".favicon-browser-tab .site-title"
            );
            if (event.target?.value.trim().length > 0) {
    ...
    let export_import_action = document.querySelector(
        "select[one]#export-import-action"
    );
    let export_import_data_type = document.querySelector(
        "select[one]#export-import-data-type"
    );
    const import_steps = document.querySelector('[aria-name="import-steps"]');
    if (export_import_data_type) {
        export_import_data_type = Choice(
            export_import_data_type,
            [
                {
                    value: "member",
                    label: '<i class="fa-duotone fa-user-tie"></i> Member(s)',
                },
                {
                    value: "committe",
                    label: '<i class="fa-duotone fa-users-viewfinder"></i> Committe(s)',
                },
            ],
            { i18next: i18next }
        ).disable();
        choices.push(export_import_data_type);
        export_import_data_type?.passedElement?.element.addEventListener(
            "change",
            (event) => {
                if (event.target?.value?.trim()?.length > 0) {
                ...
                const action =
                    export_import_action?.passedElement?.element?.value;
                if (action === "import") {
                    if (exim_submit) {
                ...
                const data_imports =
                    import_steps.querySelectorAll("[data-import]");
                data_imports.forEach((data_import) => {
                    if (
                        action === "import" &&
                        data_import.getAttribute("data-import") ===
                            event.target.value
                    ) {
                        console.log(action);
    ...
    if (export_import_action) {
        export_import_action = Choice(
            export_import_action,
            [
                {
                    value: "export",
                    label: '<i class="fa-duotone fa-cloud-arrow-up"></i> Export',
                },
                {
                    value: "import",
                    label: '<i class="fa-duotone fa-cloud-arrow-down"></i> Import',
                },
            ],
            { i18next: i18next }
        );
        choices.push(export_import_action);
        export_import_action?.passedElement?.element.addEventListener(
            "change",
            (event) => {
    ...
    const exim_address = {};
    const import_columns = document.querySelectorAll(
        'select[one][data-select="import-column"]'
    );
    import_columns.forEach((import_data) => {
        const column = choices.push(
            Choice(
                import_data,
                [
    ...
    if (Object.keys(exim_address).length > 0) {
        addressDynamicSelection(
            exim_address?.division,
            exim_address?.district,
            exim_address?.upzilla,
            exim_address?.union,
            exim_address?.ward
        );
    }
    ...
    function reloadChoiceTranslations(choices, i18next) {
        if (typeof choices === "object" && typeof updateChoice === "function") {
            choices.forEach(async (choice) => {
                const placeholder = choice?.passedElement?.element
                    ?.getAttribute("place-i18n")
                    ?.trim();
