            ...
            if(table && tr) {
                ...
                if(tr && data_single && data_basecol){
                    ...
                    if(data_name_col?.firstChild || item > 0){
                        if(data_single == 'user' && data_name_col.querySelector('a')){ // If current user
                            ...
                            notification.options = {
                                ...
                                description: `Are you sure to delete the ${data_single}: <strong>${data_name_col?.firstChild ? data_name_col.firstChild.data : item}</strong> ${data_basecol_optional ? '('+data_basecol_optional+')' : ''}`
                            }
                        }
                    }
                } else {
...
document.addEventListener('click', (event) => {
    ...
    if(event.target.tagName === 'BUTTON' && event.target.id === 'data-bulk-action') {
        let aria_bulk_action    = event.target.closest('[aria-name="bulk-action"]');
...
if(data_bulk_options){
    ...
    if(page.classList.contains('posts')){
        options.push({value: 'category', customProperties: {i18next: 'action.category'}});
    }
...
document.addEventListener('click', async (event) => {
    if(event.target.tagName === 'LI' || event.target.tagName === 'DIV'){
        ...
        const tr_default    = html?.querySelector('[aria-name="dbdata-row"] tbody tr');
...
document.addEventListener('keyup', event => {
    if(event.target?.tagName === 'INPUT' && event.target.name === 'site-title'){
